...
def translate_batch_original(args, model, batch, eos_idx=3, beam_size=10, max_length=200,
    ...
    reacts_graph_num, product_graph_num, gt_atom_idx, gt_edge_idx, pro_batch_adj, rea_batch_adj = batch
    ...
    snorm_n = torch.cat(tab_snorm_n).sqrt().to(args.device)
    # Encoder:
    with torch.no_grad():
        encoder_out, node_features, edge_feature, batch_graph_embed_z = model.cross_encoder(src, (product_graph,snorm_n), product_graph_num,
                                                                       adj_3d=None)
        atom_rc_scores = model.atom_rc_identifier(node_features)
    ...
    for step in range(0, max_length):
        ...
        with torch.no_grad():
            outputs, attn = model.decoder(src_repeat, inp, memory_bank_repeat, None,
    ...
    return pred_tokens, pred_scores, atom_rc_scores, bond_rc_scores
