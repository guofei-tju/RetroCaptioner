...
* `--checkpoint`: The specific checkpoint file to use for validation. Replace `{training_step}` with the appropriate training step number. We provide a checkpoint trained on uspto50k datasets, which takes about 10 days on one 2080Ti GPUs. You can download the checkpoint here <https://drive.google.com/drive/folders/12gNpyfM6zZJlaoHsL-2-Jwmt3qoU1_om?usp=sharing>.
